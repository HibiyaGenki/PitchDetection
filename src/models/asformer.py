...
class AttLayer(nn.Module):
    ...
    def _block_wise_self_att(self, q, k, v, mask):
        ...
        padding_mask = torch.cat(
            [
                torch.ones((m_batchsize, 1, L)).to(self.device) * mask[:, 0:1, :],
                torch.zeros((m_batchsize, 1, self.bl * nb - L)).to(self.device),
            ],
            dim=-1,
        )
    ...
    def _sliding_window_self_att(self, q, k, v, mask):
        ...
        k = torch.cat(
            [
                torch.zeros(m_batchsize, c2, self.bl // 2).to(self.device),
                k,
                torch.zeros(m_batchsize, c2, self.bl // 2).to(self.device),
            ],
            dim=-1,
        ...
        v = torch.cat(
            [
                torch.zeros(m_batchsize, c3, self.bl // 2).to(self.device),
                v,
                torch.zeros(m_batchsize, c3, self.bl // 2).to(self.device),
            ],
            dim=-1,
        ...
        padding_mask = torch.cat(
            [
                torch.zeros(m_batchsize, 1, self.bl // 2).to(self.device),
                padding_mask,
                torch.zeros(m_batchsize, 1, self.bl // 2).to(self.device),
            ],
            dim=-1,
        ...
        k = torch.cat(
            [k[:, :, i * self.bl : (i + 1) * self.bl + (self.bl // 2) * 2] for i in range(nb)], dim=0
        ...
        padding_mask = torch.cat(
            [padding_mask[:, :, i * self.bl : (i + 1) * self.bl + (self.bl // 2) * 2] for i in range(nb)], dim=0
        )  # of shape (m*nb, 1, 2l)
...
class MultiHeadAttLayer(nn.Module):
    def __init__(self, q_dim, k_dim, v_dim, r1, r2, r3, bl, stage, att_type, num_head, device):
        super(MultiHeadAttLayer, self).__init__()
        #         assert v_dim % num_head == 0
        self.conv_out = nn.Conv1d(v_dim * num_head, v_dim, 1)
        self.layers = nn.ModuleList(
            [
                copy.deepcopy(AttLayer(q_dim, k_dim, v_dim, r1, r2, r3, bl, stage, att_type, device=device))
...
class ConvFeedForward(nn.Module):
    def __init__(self, dilation, in_channels, out_channels):
        ...
        self.layer = nn.Sequential(
            nn.Conv1d(in_channels, out_channels, 3, padding=dilation, dilation=dilation), nn.ReLU()
        )
...
class AttModule(nn.Module):
    def __init__(self, dilation, in_channels, out_channels, r1, r2, att_type, stage, alpha, device):
        ...
        self.att_layer = AttLayer(
            in_channels, in_channels, out_channels, r1, r1, r2, dilation, att_type=att_type, stage=stage, device=device
        )  # dilation
...
class Encoder(nn.Module):
    def __init__(
        self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, att_type, alpha, device
    ):
        super(Encoder, self).__init__()
        self.conv_1x1 = nn.Conv1d(input_dim, num_f_maps, 1)  # fc layer
        self.layers = nn.ModuleList(
            [
                AttModule(2**i, num_f_maps, num_f_maps, r1, r2, att_type, "encoder", alpha, device)
...
class Decoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, att_type, alpha, device):
        ...
        self.layers = nn.ModuleList(
            [
                AttModule(2**i, num_f_maps, num_f_maps, r1, r2, att_type, "decoder", alpha, device=device)
                for i in range(num_layers)  # 2 ** i
            ]
        )
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
...
class ASFormer(nn.Module):
    def __init__(
        self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, device
    ):
        """num_decoders: number of decoders
        num_layers: number of layers in each encoder and decoder
        r1: r1 in the paper
        r2: r2 in the paper
        num_f_maps: number of feature maps
        input_dim: input dimension
        num_classes: number of classes
        channel_masking_rate: channel masking rate
        device: device
